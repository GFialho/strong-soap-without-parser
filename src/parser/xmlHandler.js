...
class XMLHandler {
  ...
  jsonToXml(node, nsContext, descriptor, val) {
    ...
    if (descriptor instanceof AttributeDescriptor) {
      ...
      if (descriptor.form === "unqualified") {
        ...
        let mapping = declareNamespace(
          nsContext,
          node,
          descriptor.qname.prefix,
                ...
                if (descriptor.refOriginal) {
                  if (descriptor.refOriginal.typeDescriptor) {
                    if (descriptor.refOriginal.typeDescriptor.inheritance) {
                      let extension =
                        descriptor.refOriginal.typeDescriptor.inheritance[
                          child.type
                        ];
                      if (extension) {
      ...
      for (let p in attrs) {
        ...
        if (p === this.options.xsiTypeKey) {
          ...
          if (schema) {
            ...
            xsiTypeDescriptor =
              xsiTypeInfo && xsiTypeInfo.describe({ schemas: this.schemas });
          }
          break;
        }
      }
    ...
    if (attrs != null && typeof attrs === "object") {
      for (let p in attrs) {
        ...
        if (p === this.options.xsiTypeKey) {
          ...
          node.attribute(
            "xsi:type",
            prefix ? prefix + ":" + xsiType.name : xsiType.name
          );
          continue;
        }
        ...
        if (childDescriptor == null) {
          if (this.options.ignoreUnknownProperties) continue;
            ...
            childDescriptor = new AttributeDescriptor(
              QName.parse(p),
              null,
              "unqualified"
            );
  ...
  static createSOAPEnvelopeDescriptor(prefix, nsURI, parameterDescriptor) {
    ...
    var envelopeDescriptor = new ElementDescriptor(
      new QName(nsURI, "Envelope", prefix),
      null,
      "qualified",
      false
    );
    ...
    if (parameterDescriptor && parameterDescriptor.faults) {
      ...
      var detailDescriptor = new ElementDescriptor(
        nsURI,
        "detail",
        null,
        "qualified",
...
function getSoap11FaultErrorMessage(faultBody) {
  ...
  var faultcode =
    selectn("faultcode.$value", faultBody) || selectn("faultcode", faultBody);
  if (faultcode) {
    //soap 1.1 fault
    errorMessage = " ";
...
function getSoap12FaultErrorMessage(faultBody) {
  ...
  if (code) {
    ...
    var value =
      selectn("Value.$value", faultBody) || selectn("Value", faultBody);
    if (value) {
      errorMessage = errorMessage + " " + "Value: " + JSON.stringify(value);
    }
    var subCode =
      selectn("Subcode.$value", faultBody) || selectn("Subcode", faultBody);
    if (subCode) {
      errorMessage = errorMessage + " " + "Subcode: " + JSON.stringify(subCode);
    }
    var reason =
      selectn("reason.$value", faultBody) || selectn("Reason", faultBody);
    if (reason) {
